...
class DiplomRiskAdapter:
    def __init__(self, config: DiplomRiskConfig, predictions: pd.DataFrame):
        ...
        self._by_ticker = {
            ticker: group.sort_values("decision_date").reset_index(drop=True)
    ...
    def _normalize_predictions(self, predictions: pd.DataFrame) -> pd.DataFrame:
        ...
        data = data.dropna(subset=["decision_date", "ticker"])
        data = data.sort_values(["ticker", "decision_date"]).drop_duplicates(["ticker", "decision_date"], keep="last")
    ...
    def lookup(self, symbol: str, timestamp: Any) -> dict[str, Any]:
        ...
        dates = group["decision_date"].to_numpy(dtype="datetime64[ns]")
        ...
        row = group.iloc[pos]
        age_days = max(0.0, (ts.normalize() - pd.Timestamp(row["decision_date"]).normalize()).total_seconds() / 86400.0)
    ...
    def apply_to_target(
        ...
        *,
        ...
        for idx, item in zip(active_ids, lookups, strict=True):
            ...
            elif weight < 0.0 and self.config.short_bonus_weight_pct > 0.0:
